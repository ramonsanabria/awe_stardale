...
aligment_folder="/disk/scratch1/ramons/asses_representations/evaluate/word_dist_zsc/same_diff/"
...
def load_word_alignments(path_alignments):
    ...
    with open(path_alignments) as inputfile:
        for word_line in inputfile.readlines():
            if("french" in path_alignments):
                file_id = word_line.strip().split("_")[1]+"_"+word_line.strip().split("_")[2]
            start = float(word_line.strip().split("_")[-1].split("-")[-2])/100
            end = float(word_line.strip().split("_")[-1].split("-")[-1])/100
            word = str(word_line.strip().split("_")[0])
...
with NpyAppendArray(feature_path) as npaa, open(word_path,"w") as wordfile:
    ...
    path_aligments=osp.join(aligment_folder,setname+".samediff.list")
