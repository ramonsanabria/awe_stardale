...
aligment_folder="/disk/scratch1/ramons/data/zerospeech/sse_ramons/"
    ...
    with open(path_alignments) as inputfile:
        for word_line in inputfile.readlines():
            ...
            if(file_id not in all_words):
                all_words[file_id] = [(start, end, word, False)]
            else:
                list_segments = all_words[file_id] 
                list_segments.append((start, end, word, False))
...
pooling_type=sys.argv[5]
feature_pooled_dir=sys.argv[9]

if("sub" in pooling_type):
    subsample_value=int(pooling_type.replace("sub",""))
    pooling_type="sub"
tsv_dir=sys.argv[6]
nshard=int(sys.argv[7])
feat_dir=sys.argv[8]

pathlib.Path(feature_pooled_dir).mkdir(parents=True, exist_ok=True)

feature_path=feature_pooled_dir+"/"+setname+"_"+subset+"_features_"+pooling_type+".npy"
word_path=feature_pooled_dir+"/"+setname+"_"+subset+"_features_"+pooling_type+".wrd"
...
with NpyAppendArray(feature_path) as npaa, open(word_path,"w") as wordfile:
    ...
    path_aligments=osp.join(aligment_folder,setname,subset+".ngram")    
    ...
    for r in range(nshard):
        ...
        for idx, key in tqdm.tqdm(enumerate(shard_ids), total=len_shard):
            ...
            if(setname == "xitsonga"):
                ...
                filename = key
            elif(setname == "libri" or setname == "dev-clean" or setname == "test-clean"):
                filename = key
            elif(setname == "french"):
                filename = key
            ...
            for word_index in range(len(alignments_file)):
                ...
                start, end, word, _ = alignments_file[word_index]
                alignments_file[word_index] = (start, end, word, True)
                ...
                if(word == "" or word == "SIL"):
                    ...
                    if(setname == "xitsonga"):
                        ...
                        spk_id = key.split("_")[2]+"_"+key.split("_")[0]+"-"+key.split("_")[1]+"-"+key.split("_")[-1]
                        npz_id = word + "_" + spk_id+"_%06d-%06d" % (int(round(start*100)),int(round(end*100)) + 1)
                    elif(setname=="librispeech" or setname == "dev-clean_150" or setname == "dev-clean" or setname == "test-clean" or setname == "dev-other" or  setname == "test-other" or setname == "libri" or setname == "mboshi" or setname == "libri_all" or setname == "libri_dev"):
                        spk_id = key
                        npz_id = word + "_" + spk_id+"_%06d-%06d" % (int(round(start*100)),int(round(end*100)) + 1)
                    else:
            ...
            word_alignments[key]=alignments_file
